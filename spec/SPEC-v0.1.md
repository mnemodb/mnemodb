...
A `##` heading without a `type:` prefix is an **untyped entry** — legal (P5), treated as `type: note` with all defaults.
...
|---|---|---|---|
| *(id)* | 4–26 chars, `[a-z0-9]`, unique in store; first token after `mnemo` | — | Stable identity for revision & merge. Short random lowercase IDs (the engine generates 8-char `[a-z0-9]`); stable once assigned |
...
| `owner` | free-form token (a person, a team, an agent id) | — | Who is **accountable** for this memory. Distinct from `src`, and **not** a trust input (§10) |
...
5. `owner` (§5.3) records accountability, **not** authority: conforming readers MUST NOT let it affect trust ordering, supersession, or what may be pinned. Otherwise a tool-sourced entry claiming `owner: security-team` would promote itself, which is exactly the escalation `src` exists to prevent. Ownership answers *who should look at this*, never *whose word wins*.
6. Because `src` is recorded on every entry, a conforming implementation can answer "what did this source write?" — the containment question after a tool or session turns out to be compromised. `mnemo trace <src>` is the reference implementation; matching SHOULD be hierarchical (`tool` covers every `tool/<session>`) and SHOULD normalize case and Unicode form, so a non-canonical `Tool/Session-ABC` cannot evade a trace.
